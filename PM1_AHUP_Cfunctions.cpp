...
double logcond_Xi (const uword &Xi_i, const uword &caso, const uword &curlab, const uword &newlab, const rowvec &pi_new, const uword &i, const double &I, const double &L, const uvec &Mell, const vec &Vartheta, umat W, uvec Xi, mat Pi, const mat &Pe)
{
...
void MCMC (const double &a_psi, const double &b_psi, uvec Xi, const uvec &Mell, const mat &Pe, const uword &nburn, const uword &nsams, const uword &nskip, const uword& ndisp, string path)
{
     ...
     for (s = 1; s <= S; ++s) {
          ...
          if ((uword)s > nburn && (uword)s % nskip == 0) {
               ...
               Xi_chain << "\n";
               ll_chain << ll_iter_pro(I, L, Mell, Vartheta, W, Xi, Pi, Pe) << "\n";
          }
          // display
          if ((uword)s % ndisp == 0) {
               vec r = myallelic(Xi);
