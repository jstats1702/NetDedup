...
loc <- "C:/Users/Juan Camilo/Dropbox/PAPERS/projects/RL1/code/"
#loc <- "C:/RL1_paper/"
...
###### paths
path.code <- loc # paste0(loc, "code/")
path.outs <- loc # paste0(loc, "outputs/")
...
prior.labs <- c("UP","AHUP","ABP1", "ABP1", "ABP2", "EPP1", "EPP1", "EPP2", "NBNBP","NBDP")

##### histograms
vals <- c(1,4,5,7,8,9,10)
for (j in vals) {
     #index <- which(vals == j)
     pdf(paste0(path.outs, "prior_sim_", prior.labs[j], ".pdf"), width = 5, height = 5, pointsize = 20)
     par(mfrow = c(1, 1), mar = c(4, 4, 1, 2) - 1.0, mgp = c(2, 1, 0), oma = c(0, 0, 0, 0))
     #windows(width = 20, height = 10)
     #par(mfrow = c(2, 5), mar = c(4, 3, 3, 2) + 1.0, mgp = c(2, 1, 0), oma = c(0, 0, 0, 0))
     #hist(S.data[,j], freq = F, breaks = 25, xlim=c(0,I), ylim = range(S.distr, na.rm = T), cex.axis = 0.5, las = 1,  xlab = "Number of singletons", ylab = "Probability", main = paste0(prior.labs[j], "\n Mean = ", round(ES[j], 1)))
     hist(S.data[,j], freq = F, breaks = 25, xlim=c(0,I), ylim = range(S.distr, na.rm = T), cex.axis = 0.9,  xlab = "Number of singletons", ylab = "Probability", main = " " )
     dev.off()
}
